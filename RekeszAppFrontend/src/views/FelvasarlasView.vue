...
<template>
  ...
  <div class="grid">
    ...
    <div class="card history-card">
      <div class="section-head"><div><h2>{{datum===today()?'Mai felvásárlások':`${fmtDate(datum)} felvásárlásai`}}</h2><span class="muted">{{items.length}} találat</span></div></div>
      <p v-if="betolt">Betöltés…</p>
      <div v-else class="felv-grid">
        <button v-for="item in items" :key="item.id" type="button" class="felv-tile" @click="nyitFelvDetail(item)">
          <div class="felv-tile-photo" :class="{'no-photo':!item.zoldsegKepUrl}" :style="csempeStilus(item.zoldsegId)">
            <div class="felv-tile-overlay">
              <span class="felv-tile-name">{{item.zoldsegNev}}</span>
              <span class="felv-tile-info">{{item.mennyiseg}} {{item.rekeszTipus}} · {{item.egysegar!=null?item.egysegar+' Ft':'ár nélkül'}}</span>
            </div>
            <span v-if="!item.zoldsegKepUrl" class="felv-tile-placeholder">🥬</span>
          </div>
        </button>
        <div v-if="!items.length" class="ures">Nincs a keresésnek megfelelő felvásárlás.</div>
      </div>
    </div>
  </div>
